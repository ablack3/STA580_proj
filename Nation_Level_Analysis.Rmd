...
First lets take a look at the difference in proportions for presence of personal doctor.
```{r}
...
PD.pd <- c(PD.pd, PD.propdif(ST_PD_ay, 0.05))
 ...
 Persdoc.pdif.95CI <- matrix(PD.pd, nrow = 6, ncol = 3, byrow = TRUE, dimnames = list(c("2011", "2012", "2013", "2014", "2015", "All"), c("Lower.Bound", "Point.Estimate", "Upper.Bound")))
 
barcenters <- barplot(Persdoc.pdif.95CI[,2], ylim = c( min(Persdoc.pdif.95CI[,1]) - 0.001  , max(Persdoc.pdif.95CI[,3]) ), xpd = FALSE )
barcenters
