---
title: "Nation Level Report - Reorganized"
...
#### Tests of General Association and Rank Correlation
  ...
  The Pearson chi-squared tests confirmed the presence of an association between insurance status and having a personal doctor for every year from 2011 through 2015. Furthermore, the Kendall's tau-b and Goodman and Kruskal's gamma statistics confirm that the direction of this association is towards insured individuals being more likely to have a personal doctor. Table 1 lists the Pearson test-statistics and their associated p-valus for each year and for all years combined. The large test statistics and correspondingly low p-values indicate the association is highly statistically significant. Table 2 reports the calculated rank correlation statistics, their confidence intervals, their test-statistics and their p-values for all years combined. For information on the rank correlation statistics for specific years see table A1 in the [appendix](#pearson-and-concordance-statistics-tables). Again, the large test-statistics and low p-values indicate that the positive correlation between insurance status and having a personal doctor is highly *statistically* significant. The difference in the magnitude of the tau-b and gamma statistics is due to the way they are calculated. The gamma statistic looks at the difference between the number of concordant and discordant pairs relative to the total number of *only discordant or concordant* pairs. So ties do not effect the gamma statistic. On the other hand, the tau-b statistic does take ties into account. For more information on the performance of the Pearson chi-squared test and the calculation of the rank correlation statistics please see the [appendix](#example-nation-level-analysis-calculations).
...
rcts.PDay <- rcts.PD[11:12,2:6]
rownames(rcts.PDay) <- c("Tau-b", "Gamma")

pandoc.table(rcts.PDay, caption = "**Table 2**: The rank correlation statistics for the relationship between insurance status and having a personal doctor for all years combined. The positive tau-b and gamma statistics indicate that having insurance is correlated with having a personal doctor. While the large test-statistics and p-values of virtually zero indicate that this correlation is highly statistically significant, the magnitude of the tau-b statistic indicates that the strength of the correlation is moderate.", split.table = Inf)
...
<br>
  ...
  The association between insurance status and doctor utilization was investigated both for all individuals and for individuals who reported having a personal doctor. For both of these groups the Pearson chi-squared test confirmed the presence of an association between insurance status and doctor utilization, while the rank correlation statistics confirmed that the direction of this association was towards individuals with insurance having a general health checkup more recently. Table 3 reports the Pearson chi-squared statistics for all individuals and for only individuals who reported having a personal doctor, for all years combined. For information on individual years see tables A2 and A3 in the [appendix](#pearson-and-concordance-statistics-tables). The large test-statistics and p-values of virtually zero indicate that the association between insurance status and doctor utilization is highly statistically significant, both when looking at all individuals and only individuals with a personal doctor. Table 4 reports the rank correlation statistics for all individuals and for only individuals who reported having a personal doctor when looking at all years combined. For information the concorance statistics for specific years see tables A4 and A5 in the [appendix](#pearson-and-concordance-statistics-tables). The large test-statistics and p-values of approximately zero indicate that the positive correlation between insurance status and doctor utilization is highly statistically significant. However, the relatively small magnitudes of the tau-b estimates indicate that the strength of this correlation is not necessarily very strong. Moreover, the strength of the correlation is weaker when considering only individuals who reported having a personal doctor. 
...
pcs.CUay <- rbind(pcs.CU[5,],pcs.CUgPD[5,])
rownames(pcs.CUay) <- c("All Individuals", "Individuals with a Personal Doctor")

pandoc.table(pcs.CUay, caption = "**Table 3**: The Pearson chi-squared test-statistics for the general association between insurance status and time since last general health checkup (doctor utilization). The large test-statistics and p-values of virtually zero indicate that the association between insurance status and doctor utilization is highly statistically significant.", split.table = Inf)
...
#### Absolute and Relative Difference of Proportions
  ...
  In the previous sections we showed that there is a positive correlation between having insurance and having a personal doctor. Table 5 lists the estimated absolute and relative differences in proportion between individuals in the insured group that have a doctor and individuals in the uninsured group that have a doctor. Table 5 only contains the statistics calculated for all years combined. For specific years see tables A6 and A7 in the [appendix](#proportional-differences). Figures 4 and 5 graphically shows the absolute and relative differences in proportions for each year. The line segments on top of each bar show the 95% confidence intervals. Note that the y-axis in figure 4 only covers a small range, so although the differences in proportions may decrease from 2011 to 2015, the magnitude of this decrease is small. Additionally, all the confidence intervals overlap except for those for 2011 and 2015, and even these two confidence intervals are very close. Moreover, in figure 5 when we look at the relative differences for each year we do see the same decreasing trend. Overall, individuals with insurance were estimated to be around 2.13 times more likely to have a personal doctor than individuals without health insurance.
...
pdif.PDay <- rbind(Persdoc.pdif.95CI[6,], PD.RR[6,])
rownames(pdif.PDay) <- c("Absolute Difference", "Relative Difference")

pandoc.table(pdif.PDay, caption = "**Table 5:** 95% Confidence intervals for the absolute and relative differences between the proportion of insured individuals with a doctor and uninsured individuals with a doctor.", digits = 5)
...
<br>
  ...
  Table 6 shows the absolute and relative differences between the proportion of insured individuals that have had a checkup within a year and the proportion of uninsured individuals that have. For information on specific years see tables A8 - A11 in the [appendix](#proportional-differences). The absolute difference in proportions is slightly less when considering only individuals with a doctor, about .22 compared to .34 when looking at all individuals. Similarily, the relative difference was less when considering only individuals that had a personal doctor compared to all individuals. When considering only individuals that had a doctor, insured indiviudals were 1.37 times as likely to have has a checkup within a year. On the other hand, when considering all individuals, those with insurance were 1.84 times as likely to have had a checkup within a year compared to those without insurance. Figures 6 and 7 show the absolute and relative differences for each year. From these figures it appears that the difference between insured and uninsured individuals decreased from 2011 to 105. This reduction, however, was small.
...
pdif.DU1ay <- cbind(c("Absolute", "Relative", "Absolute", "Relative"), pdif.DU1ay)
rownames(pdif.DU1ay) <- c("All","", "Personal Doctor Only", "")

pandoc.table(pdif.DU1ay, caption = "**Table 6:** 95% Confidence intervals for the absolute and relative differences between the proportion of insured individuals who have had a checkup within a year and the proportion of uninsured individuals that have.", digits = 5, split.table = Inf)
...
pdif.DU5ay <- rbind(round(DU[6,7:9], digits = 5), 
                    round(DU[6,10:12], digits = 4), 
                    round(DUgPD[6,7:9], digits = 5),
                    round(DUgPD[6,10:12], digits = 4))
pdif.DU5ay <- cbind(c("Absolute", "Relative", "Absolute", "Relative"), pdif.DU5ay)
rownames(pdif.DU5ay) <- c("All","", "Personal Doctor Only", "")

pandoc.table(pdif.DU5ay, caption = "**Table 7:** 95% Confidence intervals for the absolute and relative differences between the proportion of uninsured individuals who have not had a checkup within five years and the proportion of insured individuals that have not.", digits = 5, split.table = Inf)
...
```{r, echo=F, results="asis"}
pandoc.table(rcts.PD, caption = "**Table A1**: The rank correlation statistics for the relationship between insurance status and having a personal doctor. The positive tau-b and gamma statistics indicate that having insurance is correlated with having a personal doctor. While the large test-statistics and p-values of virtually zero indicate that this correlation is highly statistically significant, the magnitude of the tau-b statistic indicates that the strength of the correlation is moderate.", split.table = Inf)
```
...
```{r, echo=F, results="asis"}
pandoc.table(rcts.CU, caption = "**Table A4**: The rank correlation statistics for the relationship between insurance status and time since last general health checkup (doctor utilizationt). The positive tau-b and gamma statistics indicate that having insurance is correlated with having a checkup more recently. While the large test-statistics and p-values of virtually zero indicate that this correlation is highly statistically significant, the magnitude of the tau-b statistic indicates that the strength of the correlation is not very strong.", split.table = Inf)
```
...
```{r, echo=F, results="asis"}
pandoc.table(PD.RR, caption = "**Table A7:** 95% Confidence intervals for the relative differences between the proportion of insured individuals with a doctor and uninsured individuals with a doctor.", digits = 5, split.table = Inf)
```
...
```{r, echo=F, results="asis"}
pandoc.table(DUgPD[,4:6], caption = "**Table A11:** 95% Confidence intervals for the relative differences between the proportion of individuals who have had a checkup within a year, only considering individuals who reported having a personal doctor. Note that although the relative differences are close from year to year they do generally decrease from 2011 to 2015.", digits = 5, split.table = Inf)
```
...
```{r, echo=F, results="asis"}
pandoc.table(DU[,7:9], caption = "**Table A12:** 95% Confidence intervals for the absolute differences between the proportion of uninsured individuals who have not had a checkup within five years and the proportion of insured individuals that have not.", digits = 5, split.table = Inf)
```
...
```{r, echo=F, results="asis"}
pandoc.table(DUgPD[,7:9], caption = "**Table A13:** 95% Confidence intervals for the absolute differences between the proportion of individuals who have not had a checkup within five years, only considering individuals who reported having a personal doctor.", digits = 5, split.table = Inf)
```

***
<br>
```{r, echo=F, results="asis"}
pandoc.table(DU[,10:12], caption = "**Table A14:** 95% Confidence intervals for the relative differences between the proportion of uninsured individuals who have not had a checkup within five years and the proportion of insured individuals that have not.", digits = 5, split.table = Inf)
```
...
```{r, echo=F, results="asis"}
pandoc.table(DUgPD[,10:12], caption = "**Table 16:** 95% Confidence intervals for the relative differences between the proportion of individuals who have not had a checkup within five years, only considering individuals who reported having a personal doctor.", digits = 5, split.table = Inf)
```
